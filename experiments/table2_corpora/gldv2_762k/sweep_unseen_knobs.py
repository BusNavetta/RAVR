...
def default_grid(panel_queries: int) -> list[tuple[int, int]]:
    """(calibration queries, hard-negative depth) cells.

    Three families, chosen so each knob is isolated and then crossed:

    * the query sweep at the default depth -- knob 1 alone;
    * the depth sweep at the panel's query count -- knob 2 alone;
    * two iso-coverage bands, where `Q * (k + 3d)` is held constant. Those are
      the cells that decide whether the two knobs are interchangeable.
    """
    q = panel_queries
    # the panel's own cell first: it is the reference every diagnostic is
    # compared against, so it has to exist before the others are read
...
# --------------------------------------------------------------------------
# depth plumbing -- `mining_depth.install` never uninstalls, and this sweep
# walks back to the default between cells
# --------------------------------------------------------------------------

def set_depth(depth: int) -> str:
    """Force the mining depth for the next calibration; return the state tag."""
...
# --------------------------------------------------------------------------
# evaluation -- one chunked pass over the gallery scores every allocation
# --------------------------------------------------------------------------

class Evaluator:
    """Teacher Recall@10 for a batch of allocations, without packing an index.

    The score of item `i` under prefix option `o` does not depend on what any
    other item was given, so the five options can be scored once per chunk of
    the gallery and every allocation is then a gather along the option axis.
    Inverse norms are rounded to float16 first, because that is what the packed
    index stores; the run checks its own numbers against the panel's before it
    trusts any of this.
    """
...
def unseen_fallback(ravr: np.ndarray, table: np.ndarray, coverage: np.ndarray,
    ...
    """`ravr` on seen items, the leftover slack spent on unseen ones by `table`.

    This is exactly what `_selections` builds for `ravr_unseen_rd`, with the
    distortion table left as a parameter so the same construction can be tried
    with the workload-aware query-IP loss.
    """
...
def risk_diagnostics(state: dict, queries: int, depth: int,
    ...
    """What a wider mining does to the signal, not just to the coverage.

    `coverage.sum() / Q` is the mean size of the mined set, so it is also the
    dilution factor: Ordinal-KL splits half the mass over the negatives of each
    cutoff, so a negative's weight is `0.5 / negatives` and doubling the window
    halves it. The concentration measures say whether the risk that survives is
    still carried by a few items or has spread out.
    """
...
def main() -> None:
    ...
    # Resumable: a cell already in the report is kept, so a second invocation
    # can add cells (an extended query pool, say) without re-running the rest.
    if out.exists():
